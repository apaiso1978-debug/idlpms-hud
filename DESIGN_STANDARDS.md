...
## 21. กฏการเติมเต็มเฟรม (Edge-to-Edge Fluidity — The Frame Fill Rule)
*เพื่อการใช้พื้นที่หน้าจอให้เกิดประโยชน์สูงสุดในระบบบริหารจัดการ*

*   **Iron Rule:** ห้ามใช้ `max-width` (เช่น `max-w-7xl` หรือ `max-w-[1200px]`) ในหน้า Functional Page (Dashboard, Matrix, Planning)
*   **Fluid scaling:** เนื้อหาต้องขยายตัวเต็มความกว้างของเฟรม (Container `w-full`) เสมอ
*   **Dynamic Column Grid:** เมื่อพื้นที่หน้าจอกว้างขึ้น (เช่น จอ Ultra-wide) ให้ใช้ระบบ Grid เพื่อเพิ่มจำนวนคอลัมน์แทนการขยายขนาด Element ให้ใหญ่จนน่าเกลียด (เช่น ใช้ `2xl:grid-cols-4`, `3xl:grid-cols-5`)
*   **The 24px Margin:** แม้จะขยายเต็มจอ แต่ต้องรักษาขอบซ้าย-ขวาไว้ที่ **24px (`px-6`)** ตามกฎข้อ 20 เสมอ
